...
<script type="text/javascript" src="${mRoot}/static/js/jfb/base/iscroll.5.1.3.js?v=1.0"></script>
	...
	$('#content').CreateController({
		...
		init: function() {
			...
			Shop.productCategorys(function(data){
				...
				$.loadImage(); 
				setTimeout(function(){
					scroll_left = new IScroll('#view_left', {click:true, checkDOMChanges: true});
				}, 100); 
			});

			// view_right
			Product.search({'page':1}, function(page){
				that.tmpl({
					dom: that.view_right,
					v: $('#tmpl_right').html(),
					datas: page
				...
				$.loadImage();
				setTimeout(function(){
					scroll_right = new IScroll('#view_right', {click:true, checkDOMChanges: true});
				}, 100);
			});
